...
api_address = 'http://api.exchangeratesapi.io/v1/'
...
def get_currency_exchange_values(date, old_currency_symbol, new_currency_symbol):
    ...
    url = api_address + date + '?' + api_key + '&symbols=' + old_currency_symbol + ',' + new_currency_symbol
    page_content_dict = requests.get(url).json()
    if 'success' in page_content_dict:
        if page_content_dict['success']:
...
def get_all_currencies_labels():
    url = api_address + 'symbols?' + api_key
    page_content_dict = requests.get(url).json()
    labels = []
